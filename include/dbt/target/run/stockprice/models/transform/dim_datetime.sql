  ...
  create or replace view `stockprice-433416`.`stock`.`dim_datetime`
  OPTIONS()
  as -- Crie uma CTE para extrair componentes de data e hora como strings
WITH datetime_cte AS (
  SELECT DISTINCT
    CAST(Date AS STRING) AS datetime_id,
    FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', Date) AS date_part
  FROM `stock`.`stock`.`raw_stocks`
